...
#include "i2c.h" // Include your i2c header file
...
VL6180X::VL6180X()
  : bus(NULL) //Set bus to NULL, must be set with setBus
...
void VL6180X::setBus(I2C_HandleTypeDef* i2c) {
    bus = i2c;
}
...
void VL6180X::writeReg(uint16_t reg, uint8_t value)
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(bus, address << 1, data, 3, 100);
    if (status != HAL_OK) {
        last_status = 1;
    } else {
        last_status = 0;
...
void VL6180X::writeReg16Bit(uint16_t reg, uint16_t value)
{
 uint8_t data[4];
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(bus, address << 1, data, 4, 100);
    if (status != HAL_OK) {
        last_status = 1;
    } else {
        last_status = 0;
...
void VL6180X::writeReg32Bit(uint16_t reg, uint32_t value)
{
 uint8_t data[6];
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(bus, address << 1, data, 6, 100);
    if (status != HAL_OK) {
        last_status = 1;
    } else {
        last_status = 0;
...
uint8_t VL6180X::readReg(uint16_t reg)
{
  ...
  HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(bus, address << 1, data, 2, 100);
    if (status != HAL_OK) {
        last_status = 1;
        return 0;
    } else {
     ...
     status = HAL_I2C_Master_Receive(bus, (address << 1) | 0x01, &value, 1, 100);
    if (status != HAL_OK) {
        last_status = 1;
        return 0;
    } else {
...
uint16_t VL6180X::readReg16Bit(uint16_t reg)
{
  ...
  data[1] = (uint8_t)(reg & 0xFF);
	...
	HAL_I2C_Master_Transmit(bus, address << 1, data, 2, 100);
	HAL_I2C_Master_Receive(bus, (address << 1) | 0x01, (uint8_t*)&value, 2, 100);
  return value;
}
...
uint32_t VL6180X::readReg32Bit(uint16_t reg)
{
    ...
    // 1. Transmit the 16-bit register address to read from
    reg_addr[0] = (uint8_t)(reg >> 8);   // Register address high byte
    reg_addr[1] = (uint8_t)(reg & 0xFF); // Register address low byte

    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(bus, address << 1, reg_addr, 2, 100);
    if (status != HAL_OK) {
        last_status = 1;
        return 0; // Return 0 on error
    }

    // 2. Receive the 4 bytes (32 bits) of data
    status = HAL_I2C_Master_Receive(bus, (address << 1) | 0x01, received_data, 4, 100);
    if (status != HAL_OK) {
        last_status = 1;
        return 0; // Return 0 on error
    }
    ...
    // 3. Combine the 4 received bytes into a 32-bit value (big-endian)
    value = ((uint32_t)received_data[0] << 24) |
            ((uint32_t)received_data[1] << 16) |
            ((uint16_t)received_data[2] << 8)  |
